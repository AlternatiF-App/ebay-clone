...
const Checkout = () => {
  ...
  const stripeInit = async () => {
      ...
      card.current.on("change", function (event: any) {
        const queryButton:any = document.querySelector("button")
        const queryCard:any = document.querySelector("#card-error")
        queryButton.disabled = event.empty;
        queryCard.textContent = event.error ? event.error.message : "";
      });
...
const showError = (errorMsgText: any) => {
    let errorMsg: any = document.querySelector("#card-error");
  ...
  return (
    <MainLayout>
      <div id='checkout-page' className='mt-4 max-w-[1100px] mx-auto'>
        <div className='text-2xl font-bold my-4'>Checkout</div>
        <div className='relative flex items-baseline gap-4 justify-between mx-auto w-full'>
          ...
          <div id='place-order' className='relative -top-[6px] w-[35%] border rounded-lg'>
            <ClientOnly>
              <div className='p-4'>
